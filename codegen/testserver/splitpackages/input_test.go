...
// Ported from codegen/testserver/singlefile/input_test.go (TestInput): the
// oracle for [String!]! / [String!] argument unmarshaling, including the
// null-vs-empty-list distinction the split slice codec must preserve.
...
// Ported from codegen/testserver/singlefile/input_test.go
// (TestInputOmittable): the oracle for explicit-null vs. unset on
// graphql.Omittable[T] input fields. ThirdParty's unexported str field is
// reached via ThirdParty.String() here (see model/thirdparty.go) since
// split-packages keeps models in their own package.
